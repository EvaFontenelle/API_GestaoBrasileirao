...
def listar_SerieA():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT idSerieA, NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela FROM SerieA")
    dados = [
        {
            "idSerieA": row[0], 
            "NomeClube": row[1], 
...
def adicionar_ClubeA():
    dados = request.get_json(silent=True)
    if not dados:
        abort(400, description="Clube inválido ou ausente")

    # Validação de campos obrigatórios
    campos_obrigatorios = {"NomeClube", "PontosClube", "JogosClube", "SaldoGols", "VitoriasClube", "DerrotasClube", "EmpatesClube", "PosicaoTabela"}
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieA (NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela)"
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (
            dados["NomeClube"],
            dados["PontosClube"],
...
def atualizar_ClubeA(idSerieA):
    ...
    if request.method == "PUT":
        campos_esperados = {"NomeClube", "PontosClube", "JogosClube", "SaldoGols", "VitoriasClube", "DerrotasClube", "EmpatesClube", "PosicaoTabela"}
    ...
    campos_validos = {"NomeClube", "PontosClube", "JogosClube", "SaldoGols", "VitoriasClube", "DerrotasClube", "EmpatesClube", "PosicaoTabela"}
    set_clauses = []
    valores = []
...
@SerieA_bp.route("/SerieA/<int:idSerieA>", methods=["DELETE"])
def deletar_ClubeA(idSerieA):
    conn = conectar()
    cursor = conn.cursor()
