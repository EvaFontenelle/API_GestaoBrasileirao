...
def listar_SerieC():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT idSerieC, NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela FROM SerieC")
    dados = [
        {
            "idSerieC": row[0], 
            "NomeClube": row[1], 
...
def adicionar_ClubeC():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieC (NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela)"
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (
            dados["NomeClube"],
            dados["PontosClube"],
