...
def listar_SerieD():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT idSerieD, NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela FROM SerieD")
    dados = [
        {
            "idSerieD": row[0], 
            "NomeClube": row[1], 
...
def adicionar_ClubeD():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieD (NomeClube, PontosClube, JogosClube, SaldoGols, VitoriasClube, DerrotasClube, EmpatesClube, PosicaoTabela)"
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (
            dados["NomeClube"],
            dados["PontosClube"],
